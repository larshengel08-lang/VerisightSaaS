...
def _build_culture_board_attention_points(domain_averages: dict[str, float]) -> list[dict[str, str]]:
    ...
    for index, (domain_id, score) in enumerate(lowest, start=1):
        if score <= 5.2:
            ...
            reason = "contrast en spreiding vragen bestuurlijke verificatie"
            confidence = "midden"
        else:
            reason = "bestuurlijke aandacht op basis van patroon en samenhang"
...
def _generate_culture_assessment_report(
    ...
    *,
    ...
    open_text_count = sum(1 for response in responses if (response.open_text_raw or "").strip())
    open_text_safe = open_text_count >= int(methodology_payload["open_text_cluster_min_n"])
    ...
    story.append(Spacer(1, 0.12 * cm))
    story.append(Paragraph("Loep Culture Assessment - Board Baseline", STYLES["cover_sub"]))
    ...
    _append_culture_report_section(
        ...
        bullets=[
            ...
            f"Open tekst {'is' if open_text_safe else 'is niet'} veilig genoeg geclusterd voor aanvullende context.",
