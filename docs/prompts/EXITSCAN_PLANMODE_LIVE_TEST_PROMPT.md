# ExitScan Planmodus Prompt - Live Test

```text
Analyseer deze website, live omgeving en codebase en maak een uitvoerbaar planbestand `EXITSCAN_LIVE_TEST_PLAN.md` voor een volledige live test van ExitScan.

Belangrijk:
- `EXITSCAN_LIVE_TEST_PLAN.md` wordt de source of truth voor de uitvoering
- voer nog geen codewijzigingen uit
- analyseer eerst de huidige site, routes, knoppen, CTA's, dashboardinteracties en functionele flows van ExitScan
...
Scope:
- alleen ExitScan
- publieke website waar ExitScan zichtbaar is
...
- RetentieScan alleen meenemen waar vergelijking nodig is om ExitScan live duidelijk te positioneren

Doel:
Maak een plan om ExitScan live volledig te reviewen en te testen, zodat:
...
- `frontend/lib/products/exit/*`
- relevante api-routes en backend-endpoints

Vereisten voor `EXITSCAN_LIVE_TEST_PLAN.md`:
...
- risico dat ExitScan live onduidelijk of stroef voelt

Structuur van `EXITSCAN_LIVE_TEST_PLAN.md`:
1. Summary
2. Milestones
...
- maak alleen `EXITSCAN_LIVE_TEST_PLAN.md`
