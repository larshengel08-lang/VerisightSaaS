...
De drie bevindingen zijn dezelfde als vóór deze ronde, met dezelfde percentages: `paginavulling` op pagina 7 van 01 (36%), 09 (26%) en 19 (36%). Na de A1-formulering van 25-9 opnieuw gegenereerd en gerenderd: dezelfde uitkomst en per bestand hetzelfde aantal pagina's (de voorbeeldrapporten bevatten die vragen niet en veranderden niet).
...
| 5 | Namenregel Loep Vertrek; amendement A1 (tijdsanker) en A2 (hint) | `ae3c7f0c`, `cf462533`, `ca73c3d4`, `38b9713e` (A1-formulering van Lars, 25-9) |
...
- **Taak 5:** pagina twee van scenario 08 liep sinds Taak 4 over (opgelost in Taak 7); de A1-formulering "deze mensen" verwees in drie vragen naar het verkeerde meervoud. Lars koos op 25-9 het voorstel van de reviewer: in alle veertien vragen "in de periode waar deze meting over gaat" (`38b9713e`).
...
- **Taak 12, 13, 14:** een door de implementer toegevoegde grenshint op het formulier publiceerde een prijsregel die Lars niet besliste (verwijderd, beslispunt 5); verder alleen kleine punten.
...
- **Taak 17:** een meting die na de inventaris werd heropend werd toch gewist (nu hercontrole met `FOR UPDATE`); zeven Action Center-tabellen met onder meer e-mailadressen vielen buiten de opschoning; een onbekend organisatie-id gaf stil succes; foutmeldingen konden persoonsgegevens bevatten; een maandelijkse run kon tot een maand na "uiterlijk twee jaar" wissen (nu één maand vooruit, beslispunt 7).
...
- **Amendement A1/A2 en A4** (besluiten Lars) vervingen de plantekst waar het plan "ongewijzigd, beslispunt" zei; zie het amendement. A1 werd op 25-9 nog bijgesteld naar "in de periode waar deze meting over gaat". Maandelijkse in plaats van dagelijkse cron; leads en dossiers wél in de opschoning; twee juridische zinnen.
...
De punten 1, 2, 5 en 9 raken klantzichtbare tekst of een belofte; de rest is kleiner.
...
3. **Uitstroomperiode en herleidbaarheid.** De vroegste en de laatste maand kunnen elk van één persoon zijn; HR kent de maanden en de audit van 13-7 noemt `exit_month` een quasi-identificerende kolom. Accepteren, pas een periode tonen als beide randmaanden minstens twee personen hebben, of per kwartaal? Daarnaast: "vertrokken" is verleden tijd, terwijl `exit_month` bij een lopende meting een geplande maand kan zijn.
4. **Leesbaarheid op papier.** De duidingsalinea op pagina twee staat op 9,5px (de afspraak van juli is 10px voor zulke regels; 10px kost een regel op scenario 08). De hints op de besluitpagina (parkeerregel, afdeling, terugkoppeling) staan op 8,5px cursief; vergroten kost ruimte (25pt marge in het slechtste geval). In het dashboard staan die lange hints in hoofdletters (de labelstijl); een eigen hintstijl is een ontwerpkeuze.
5. **Prijsgrenzen 400 en 1.000.** "150 tot 400" en "400 tot 1.000" bevatten allebei 400; "400 tot 1.000" en "Boven 1.000" laten 1.000 dubbel lezen. Een organisatie van precies 400 past in twee omvangvakken. Voorstel: de labels in `frontend/lib/pricing.ts` ondubbelzinnig maken (het formulier volgt dan vanzelf).
6. **`BESLUIT_TEKST_MAX` naar 240.** Het rapport toont het begin van elk lang besluitveld (met melding); het dashboard waarschuwt niet dat de PDF maar 240 van de 600 tekens toont. Hint bij het veld, of een lagere limiet in het dashboard?
7. **Opschoning, vooruitblik.** De maandelijkse run schoont tot een maand vóór het einde van de termijn op, ook bij een schriftelijk afgesproken langere termijn. Past bij "uiterlijk"; alternatief is een dagelijkse run zonder vooruitblik.
8. **Opschoning, restpunten.** (a) Goedgekeurde publieke cases in `case_proof_registry` worden met de meting gewist; bewaar het bewijs van toestemming buiten de database als een case gepubliceerd wordt. (b) Telemetrie- en bewijsrijen zonder `campaign_id` (ook na het verwijderen van een meting, `on delete set null`) worden nooit opgeschoond; voorstel: twee jaar na aanmaken. (c) `action_center_support_access_events` (organisatieniveau, optionele vrije tekst) valt bewust buiten schrijfslot en opschoning. (d) Een lead wissen maakt de koppeling in de levering leeg; de operator ziet dan "handoff nog niet expliciet gekoppeld". (e) Een operator die een opgeschoonde meting heropent, moet ook `closed_at` leegmaken; anders kan de klant niet sluiten. Een heropende opgeschoonde meting blijft overal "Gegevens verwijderd".
9. **Privacyzin leads.** "Wat je via het contactformulier of in een kennismaking met Loep deelt" dekt niet expliciet de eigen notities die Loep in leerdossiers maakt (koopreden, uitkomst, lessen); de termijn klopt wel.
10. **Loep Start** (buiten deze ronde): de brugzin zegt onder 30% respons nog "daar begint het gesprek ook" naast een kernzin met "mogelijk startpunt".
11. **Uit het plan, nog open:** afspraak per afdeling alleen op papier (geen dashboardveld); parkeerregel in plaats van een tweede eigenaar (kolommen kunnen met plan 3c mee); de twee pdf's in `Loep_Docs` (`Loep onepager.pdf`, `Loep methodische verantwoording.pdf`) opnieuw exporteren uit de bijgewerkte HTML.

## Bewust niet gedaan

- **Loep Start (S1 tot en met S8):** buiten deze ronde; ook de indicatieve brugzin (beslispunt 10).
...
- **De vertrekmaand in de Vertrek-vragenlijst:** aparte vervolgtaak na de merge (amendement) Die maakt ook het tijdsanker concreet: zonder vastgelegde maanden kan een MT-lid "de periode waar deze meting over gaat" lezen als de weken waarin de vragenlijst openstond, terwijl de meting verder terugkijkt (opmerking uit de review van `38b9713e`).
