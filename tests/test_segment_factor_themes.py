...
def test_render_n5_9_bandlabel_zonder_score():
    ...
    assert "Alle onderwerpen per afdeling" not in html
...
def test_render_n10_score_en_subblok_laagste_eerst():
    ...
    sub = html[html.index("Alle onderwerpen per afdeling"):]
...
def test_navy_anchor_wijst_geen_afdeling_aan_bij_n5_9():
    ...
    assert "maar heeft 6 antwoorden" in anchor
    ...
    marketing_cell = next(c for c in _theme_cells(html)
                          if "Werkdruk en herstelruimte" in c)
    assert "Kwetsbaar punt" in marketing_cell
    assert not re.search(r"\d\.\d/10", marketing_cell)
...
def test_lege_factors_met_omitted_toont_meldregel_geen_kaal_nb():
    ...
    assert "Alle onderwerpen per afdeling" not in html    # geen subblok zonder factors
...
def test_volledige_renderer_toont_laagste_thema():
    ...
    assert "Alle onderwerpen per afdeling" in html
