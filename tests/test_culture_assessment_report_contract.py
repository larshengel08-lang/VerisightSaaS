...
def test_culture_assessment_module_exposes_definition_and_report_payloads():
    ...
    assert "executive one-pager" not in module.get_definition()["standard_outputs"]
    ...
    assert module.get_methodology_payload()["runtime_role_model"]["active_in_v1"] == [
        "admin",
        "hr_partner",
        "executive",
    ]
    assert module.get_methodology_payload()["runtime_role_model"]["future_contract_roles_not_active_in_v1"] == [
        "business_unit_lead",
        "manager_limited",
    ]
