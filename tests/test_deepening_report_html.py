...
# Aantal respondenten in de meting: de noemer waarmee elke keten begint (B14).
N_TOTAL = 39
...
def test_block_empty_when_no_triggers():
    assert _deepening_block(_agg(0, 0, 0, 0), "retention", "workload", N_TOTAL) == ""
...
def test_block_percentages_from_10():
    html = _deepening_block(_agg(14, 13, 12, 1, {"wl_recovery": 8, "wl_volume": 4}), "retention", "workload", N_TOTAL)
    assert "8 van de 12 (67%)" in html and "4 van de 12 (33%)" in html
...
def test_geen_secondary_samenvattingsregel():
    ...
    below = _deepening_block(_agg(14, 13, 12, 1, {"wl_recovery": 8}, {"wl_volume": 4}), "retention", "workload", N_TOTAL)
    assert "Daarnaast" not in below
    at5 = _deepening_block(_agg(14, 13, 12, 1, {"wl_recovery": 8}, {"wl_volume": 3, "wl_process": 2}), "retention", "workload", N_TOTAL)
    assert "Daarnaast" not in at5


def test_no_forbidden_report_words():
    html = _deepening_block(_agg(14, 13, 12, 1, {"wl_recovery": 8}), "retention", "workload", N_TOTAL).lower()
    for w in ["betrouwbaar", "aanwijz", "laag gescoord", "oorzaak"]:
        assert w not in html


def test_unknown_option_key_rendered_raw_not_crash():
    html = _deepening_block(_agg(14, 13, 12, 1, {"wl_legacy_key": 8, "wl_volume": 4}), "retention", "workload", N_TOTAL)
