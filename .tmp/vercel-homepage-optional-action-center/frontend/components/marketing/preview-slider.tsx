    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        {copy.hypotheses.map(({ title, body, question }) => (
          <div key={title} className="rounded-[1.5rem] border border-[#E8B7A6] bg-[#FAF1EC] p-5">
            <p className="text-sm font-bold text-slate-950">{title}</p>
            <p className="mt-3 text-sm leading-6 text-slate-700">{body}</p>
            <div className="mt-4 rounded-[1.15rem] border border-white/80 bg-white/90 p-4">
              <p className="text-xs font-semibold uppercase tracking-wide text-[#C96A4B]">Te toetsen vraag</p>
  ...
  return (
    <div className="marketing-preview-shell">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#C96A4B]">{copy.label}</p>
        ...
        {SLIDES.map((label, index) => (
          <button
            ...
            className={`rounded-full px-4 py-2 text-sm font-semibold transition-colors ${
              index === current
                ? 'bg-[#C96A4B] text-white'
                : 'border border-[#E5E0D6] bg-white text-slate-600 hover:border-[#C96A4B] hover:text-[#132033]'
            }`}
          >
            {label}
          </button>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.12fr)_minmax(18rem,0.88fr)]">
        ...
        <div className="grid gap-4 self-start">
          ...
          <div className="rounded-[1.6rem] border border-[#E8B7A6] bg-[#FAF1EC] p-5">
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#C96A4B]">Belangrijke nuance</p>
