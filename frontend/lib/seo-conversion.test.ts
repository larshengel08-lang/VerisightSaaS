...
describe('SEO conversion tranche', () => {
  it('keeps the homepage and support-page metadata aligned with current SEO positioning', () => {
    ...
    expect(imageUrl(aanpakMetadata.openGraph?.images)).toBeUndefined()
    expect(imageUrl(pricingMetadata.openGraph?.images)).toBeUndefined()
    expect(imageUrl(trustMetadata.openGraph?.images)).toBeUndefined()
  })
  ...
  it('generates solution and product metadata from the shared SEO contracts', async () => {
    ...
    expect(teamProductMetadata.title).toBe('TeamScan | Verisight')
