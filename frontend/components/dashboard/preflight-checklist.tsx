...
export function PreflightChecklist({
  ...
  const items: Array<{ key: string; label: string; auto: boolean; checked: boolean; note?: string }> = [
    ...
    },
    {
      ...
      note: 'Bevestig dat dashboard of rapport echt in een eerste gesprek wordt gebruikt.',
    },
    {
      key: 'email_template',
  ...
  return (
    <details className="group rounded-[22px] border border-slate-200 bg-white">
      <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-4">
        <div>
          <p className="text-sm font-semibold text-slate-900">Onboarding acceptance checklist</p>
          <p className="mt-1 text-sm text-slate-500">
            Gebruik deze lijst voor livegang, rapportuitleg en het eerste echte managementgebruik.
          </p>
        </div>
      ...
      <div className="border-t border-slate-100 px-4 py-4">
        {scanType === 'retention' ? (
          <p className="mb-4 rounded-2xl border border-emerald-100 bg-emerald-50 px-4 py-3 text-sm leading-6 text-emerald-900">
            Voor RetentieScan hoort bij livegang ook dat follow-up uitkomsten later op team- of segmentniveau kunnen
            worden vastgelegd. Zo blijft de v1.1-validatie niet afhankelijk van losse reconstructies.
          </p>
        ) : null}
          ...
          {items.map((item) => (
            <li
              ...
              className={`rounded-2xl border px-4 py-3 ${
                item.checked ? 'border-emerald-200 bg-emerald-50/70' : 'border-slate-200 bg-slate-50/70'
              } ${!item.auto && !(item.key === 'test_report' && !hasMinDisplay) ? 'cursor-pointer' : ''}`}
              onClick={() => {
            ...
            >
              <div className="flex items-start gap-3">
                ...
                >
                  {item.auto ? (item.checked ? 'âœ“' : '!') : item.checked ? 'âœ“' : 'â—‹'}
                </span>
