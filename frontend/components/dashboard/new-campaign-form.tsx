...
export function NewCampaignForm({ orgs }: Props) {
  ...
  const campaignNamePlaceholder = scanType === 'exit' ? 'ExitScan Q2 2026' : 'RetentieScan Voorjaar 2026'
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="rounded-[22px] border border-slate-200 bg-slate-50 p-4">
        ...
        <p className="mt-1 text-sm text-slate-600">Leg eerst vast voor welke klant de campagne draait en hoe de campagne in het dashboard moet terugkomen.</p>
      </div>
      ...
      <div className="rounded-[22px] border border-slate-200 bg-slate-50 p-4">
        ...
        <p className="mt-1 text-sm text-slate-600">Hier bepaal je of de campagne als ExitScan of RetentieScan wordt opgezet en of je de standaarddiepte gebruikt of een bewuste subset/add-on.</p>
      </div>

      <div className="grid gap-2 sm:grid-cols-2">
        {(['exit', 'retention'] as const).map((option) => (
          <button
            key={option}
            type="button"
            onClick={() => setScanType(option)}
          ...
          >
            <p className="text-sm font-semibold">{option === 'exit' ? 'ExitScan' : 'RetentieScan'}</p>
            <p className={`mt-1 text-sm ${scanType === option ? 'text-blue-100' : 'text-slate-500'}`}>
              {option === 'exit'
                ? 'Terugkijken op vertrek, frictie en beïnvloedbare werkfactoren.'
      ...
      <div className="rounded-[22px] border border-blue-100 bg-blue-50 p-4">
        ...
        {!hasSelectedFactorSubset && hasSegmentDeepDive ? (
          <p className="mt-2 text-xs text-blue-800">De add-on wijzigt de survey niet: alle standaardfactoren blijven actief en het rapport krijgt extra segmentanalyse.</p>
        ) : null}
      </div>
